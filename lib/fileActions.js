...
function upgradePackageJson({ pkg, source, componentName, ownerName }) {
  ...
  properties.forEach(prop => {
    ...
    if (prop.overrideValue) {
      _set(currentPkg, prop.key, Object.assign({}, currentPkg[prop.key], prop.overrideValue))
    }
    // 处理预防值，会保留用户的字段
    if (prop.fallbackValue) {
      _set(currentPkg, prop.key, Object.assign({}, prop.fallbackValue, currentPkg[prop.key]))
    }

    if (!prop.fallbackValue) return
    // 以用户的 package.json 文件为基础，避免因合并后造成排序混乱导致 review 增加负担。
    Object.keys(pkg[prop.key] || { [prop.key]: { ...prop.fallbackValue } }).forEach(k => {
      // 以顺序赋值
